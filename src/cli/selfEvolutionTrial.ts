...
type TrialProvider = "fixture" | "mimo" | "auto";
...
async function main(): Promise<void> {
  ...
  const provider = (readStringArg(args, "--provider") as TrialProvider | undefined) ?? "fixture";
  const focus = readStringArg(args, "--focus");
  const repeat = readNumberArg(args, "--repeat") ?? 1;
  const limit = readNumberArg(args, "--limit");
  const outPath = readStringArg(args, "--out") ?? path.join(".runtime", "self-evolution-trial.json");
  ...
  const result = await runSelfEvolutionTrial(selectedSamples, {
    studentId: "self-evolution-cli",
    occurredAt: "2026-04-30T00:00:00.000Z",
    diagnose: teacher.diagnose
  });

  const output = {
    provider: "self-evolution-cli",
    ...
    JSON.stringify(
      {
        ...
        painPointCounts: result.painPointCounts,
        recommendationCounts: result.recommendationCounts,
        readySkills: result.readySkills,
...
async function resolveTeacher(provider: TrialProvider): Promise<{
  ...
  if (provider === "mimo") {
    throw new Error("MiMo live trial requested, but secrets/models.env is missing MiMo teaching config.");
  }
