...
export function openCodeforcesLoginTerminal(options: LoginTerminalOptions = {}): void {
  openPlatformLoginTerminal("codeforces", options);
}

export function openPlatformLoginTerminal(
  ...
  const capability = getSubmissionPlatformCapability(submissionPlatform);
  const launcher = options.launcher ?? (spawn as unknown as TerminalLauncher);
  const child = launcher(
    "powershell.exe",
    ["-NoExit", "-Command", `oj login ${capability.loginUrl}`],
