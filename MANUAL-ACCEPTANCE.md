...
1. 打开文件，把光标放在内层 `if` 的空白缩进行。
2. 按 `Ctrl+Shift+P`，运行 `AI 做题陪练：触发自动补全`。
3. 等待灰色 Ghost Text；这是关键验收，侧栏“测试补全”预览不能替代它。
4. 若未出现，回到侧栏点击“测试补全”，查看结果究竟是 `model-empty`、`validator-rejected` 还是传输失败，并记录这一分类。
...
4. 再触发一次 1–3 行 Ghost Text；补全和追问本身不应提升 StudentSkill。
